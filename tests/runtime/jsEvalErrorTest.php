...
final class jsEvalErrorTest extends TestCase
{
  ...
  {
    $e = new jsEvalError('Boom!');
    $this->assertSame('EvalError', $e->get('name')->toStr()->value);
    $this->assertSame('Boom!', $e->get('message')->toStr()->value);

    $s = $this->callMethod($e, 'toString', []);
    $this->assertSame(Base::STRING, $s->type);
    $this->assertSame('EvalError: Boom!', $s->value);
  }

  public function testGlobalEvalErrorFunctionCreatesEvalError(): void
  {
    // Global constructor registered in Runtime::start(): "EvalError"
    $fnRef = Runtime::id('EvalError');
    $e = Runtime::call($fnRef, [Runtime::js_str('Oops')]);

    $this->assertInstanceOf(jsEvalError::class, $e);
  ...
  public function testToStringThrowsOnWrongThis(): void
  {
    ...
    try {
      ...
      // Inherits jsError::toString(), which validates "this" is an Error object
      jsEvalError::toString();
    } finally {
      Runtime::pop_context();
    }
  }
