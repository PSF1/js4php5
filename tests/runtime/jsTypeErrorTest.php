...
final class jsTypeErrorTest extends TestCase
{
  ...
  {
    $e = new jsTypeError('Wrong type');
    $this->assertSame('TypeError', $e->get('name')->toStr()->value);
    $this->assertSame('Wrong type', $e->get('message')->toStr()->value);

    $s = $this->call($e, 'toString', []);
    $this->assertSame(Base::STRING, $s->type);
    $this->assertSame('TypeError: Wrong type', $s->value);
  }

  public function testGlobalTypeErrorFunctionCreatesTypeError(): void
  {
    // Global constructor registered in Runtime::start(): "TypeError"
    $fnRef = Runtime::id('TypeError');
    $e = Runtime::call($fnRef, [Runtime::js_str('Oops')]);

    $this->assertInstanceOf(jsTypeError::class, $e);
  ...
  public function testToStringThrowsOnWrongThis(): void
  {
    ...
    try {
      ...
      jsTypeError::toString(); // inherited from jsError, validates "this"
    } finally {
      Runtime::pop_context();
    }
  }
